...
function initialiseMediaPlayer() {
	...
	mediaPlayer.addEventListener('play', function() {
		...
		changeButtonType(playPauseBtn, 'playerPause btnPlayer');
	}, false);
	mediaPlayer.addEventListener('pause', function() {
		// Change the button to be a play button
		changeButtonType(playPauseBtn, 'playerPlay btnPlayer');
	}, false);
	
	// need to work on this one more...how to know it's muted?
	mediaPlayer.addEventListener('volumechange', function(e) { 
		// Update the button to be mute/unmute
		if (mediaPlayer.muted) changeButtonType(muteBtn, 'playerUnmute btnPlayer');
		else changeButtonType(muteBtn, 'playerMute btnPlayer');
	}, false);	
...
function togglePlayPause() {
	...
	if (mediaPlayer.paused || mediaPlayer.ended) {
		// Change the button to be a pause button
		changeButtonType(playPauseBtn, 'playerPause');
		// Play the media
		mediaPlayer.play();
	}
	// Otherwise it must currently be playing
		...
		// Change the button to be a play button
		changeButtonType(playPauseBtn, 'playerPlay');
		// Pause the media
		mediaPlayer.pause();
	}
}
...
function toggleMute() {
	if (mediaPlayer.muted) {
		...
		changeButtonType(muteBtn, 'playerMute');
		// Unmute the media player
		mediaPlayer.muted = false;
	}
	else {
		// Change the button to be an unmute button
		changeButtonType(muteBtn, 'playerUnmute');
